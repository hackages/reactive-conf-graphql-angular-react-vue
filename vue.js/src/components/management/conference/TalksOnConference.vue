<template>
  <div>
    <cp-page-header :headers="['Conference', 'Talks on Conference Management']"></cp-page-header>
    <div class="section">
      <div class="container">
        <div class="columns">
          <div class="column is-4">
            <nav class="panel">
              <p class="panel-heading">Talk List</p>
          ...
          <div class="column is-4 is-offset-2">
            <nav class="panel">
              <p class="panel-heading">Conference Talks</p>
              <cp-talks-on-conference-row
              ...
              <a
                class="panel-block"
                v-if="!talksOnConference.length"
              >No talks yet on this conference</a>
            </nav>
          </div>
        </div>
      </div>
...
export default {
  ...
  methods: {
    ...
    async deleteTalk(talkId) {
      try {
        const talks = this.talksOnConference
          .map(talk => talk.id)
          .filter(id => id !== talkId);
        // TODO: Write updateTalksOnConference
        // Replace Promise.reject() by the correct call
